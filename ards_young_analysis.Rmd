...
LocalPatientClinicalCourse=read.csv("//Zisilon03/N03iam1/INTERNES/BertrandMoal/covid19/covid_19/code/4c/phase_1_1/phase2/output/october/LocalPatientClinicalCourse.csv")
LocalPatientObservations=read.csv("//Zisilon03/N03iam1/INTERNES/BertrandMoal/covid19/covid_19/code/4c/phase_1_1/phase2/output/october/LocalPatientObservations.csv")
LocalPatientSummary=read.csv("//Zisilon03/N03iam1/INTERNES/BertrandMoal/covid19/covid_19/code/4c/phase_1_1/phase2/output/october/LocalPatientSummary.csv")
...
end_date_p1=as.POSIXct(as.Date("2020-08-01"), tz = Sys.timezone())
...
LocalPatientSummary <- LocalPatientSummary %>%
  ...
  dplyr::mutate( age_group_spe = dplyr::case_when(  
    ...
    periode_group = dplyr::case_when(  
      admission_date >= start_date_p1 &  admission_date < end_date_p1 ~ "Jan_Jul",
      admission_date >= end_date_p1 &  admission_date <= start_date_p2 ~ "Aug_Dec"))
...
lab_day <- LocalPatientObservations %>%
  dplyr::filter( concept_type =="LOINC") %>%
  dplyr::group_by(patient_num,GROUP,days_since_admission) %>%
  dplyr::summarise(count_proc=n()) %>%
  dplyr::right_join(LocalPatientClinicalCourse[,c("patient_num","GROUP","days_since_admission")],by =c("patient_num","GROUP","days_since_admission"))%>%
  dplyr::mutate(count_proc=ifelse(is.na(count_proc),0,count_proc))%>%
...
write.csv(output_gen, file =paste("output/ards_young_gen_",siteid,".csv",sep = ""), row.names = FALSE, na = "")
write.csv(output_day, file =paste("output/ards_young_day_",siteid,".csv",sep = ""), row.names = FALSE, na = "")
write.csv(output_lab, file =paste("output/ards_young_lab_",siteid,".csv",sep = ""), row.names = FALSE, na = "")
...
knit: (function(inputFile, encoding) {
     rmarkdown::render(inputFile, encoding = encoding, output_dir = "output", output_file=paste("output/ards_young_",siteid,".html",sep = "")) })
