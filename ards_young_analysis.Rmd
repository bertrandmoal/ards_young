...
LocalPatientClinicalCourse=read.csv("//Zisilon03/N03iam1/INTERNES/BertrandMoal/covid19/covid_19/code/4c/phase_1_1/phase2/output/february2021/LocalPatientClinicalCourse.csv", na.strings = "1900-01-01")
LocalPatientObservations=read.csv("//Zisilon03/N03iam1/INTERNES/BertrandMoal/covid19/covid_19/code/4c/phase_1_1/phase2/output/february2021/LocalPatientObservations.csv",na.strings = '-999')
LocalPatientSummary=read.csv("//Zisilon03/N03iam1/INTERNES/BertrandMoal/covid19/covid_19/code/4c/phase_1_1/phase2/output/february2021/LocalPatientSummary.csv", na.strings = "1900-01-01")
...
###################################################### 
#######  No  NEED  TO  CHANGE THE PART BELOW #######  
######################################################  
...
pheno_ICD9=read.csv("doc/phecode_icd9_rolled.csv", sep = ",")
...
comp_class <- read.csv(paste0('doc/complication_class.csv'),sep = ";")

## load procedure 
sev_proc_icd10 <- read.csv('doc/sev_proc_icd10.csv')

## load lab 
lab_mapping <- read.csv('doc/4CE_loinc.csv')

## load medication
med_code= read.csv("doc/4CE_medication.csv")
...
plot_diag_per_week <- LocalPatientObservations %>%
  dplyr::filter( concept_type %in% c("DIAG-ICD10","DIAG-ICD9")) %>%
  dplyr::mutate(week = floor(days_since_admission/7))%>%
  dplyr::group_by(week)%>%
  distinct(patient_num, concept_code)%>%
  summarise(n_diag_pat=n())%>%
  dplyr::filter( week >=-3) %>%
  ggplot(aes(x = week, y = n_diag_pat)) + 
  geom_bar(stat="identity", color="black", position=position_dodge())+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  ylab(label = "number of couple ICD-Patient")+
  xlab(label = "week")
...
plot_no_diag_day <- LocalPatientObservations %>%
  dplyr::filter( concept_type %in% c("DIAG-ICD10","DIAG-ICD9")) %>%
  dplyr::group_by(GROUP) %>%
  dplyr::summarise(count_pat=n_distinct(patient_num)) %>%
  dplyr::right_join(output[,c("value","GROUP")],by =c("GROUP"))%>%
  dplyr::mutate(count_pat=ifelse(is.na(count_pat),0,count_pat))%>%
  dplyr::mutate(per_no=round(100*(value-count_pat)/value,2))%>%
  ggplot(aes(x = GROUP, y = per_no)) + 
  geom_bar(stat="identity", color="black", position=position_dodge())+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  ylab(label = "percentage of patient without diagnotics")
...
plot_no_proc_day <- LocalPatientObservations %>%
  dplyr::filter( concept_type %in% c("PROC-ICD10","PROC-ICD9","PROC-CPT")) %>%
  dplyr::group_by(GROUP,days_since_admission) %>%
  dplyr::summarise(count_pat=n_distinct(patient_num)) %>%
  ...
  dplyr::mutate(per_no=round(100*(value-count_pat)/value,2))%>%
  dplyr::filter(days_since_admission >= 0 & days_since_admission <15)%>%
  ggplot(aes(x = days_since_admission, y = per_no,color = GROUP, fill = GROUP)) + 
  geom_line() +
  geom_point() + 
  ylab(label = "percentage of patient without procedure")
...
plot_lab_day <- LocalPatientObservations %>%
...
plot_no_lab_day <- LocalPatientObservations %>%
  dplyr::filter( concept_type =="LAB-LOINC") %>%
  dplyr::group_by(GROUP,days_since_admission) %>%
  dplyr::summarise(count_pat=n_distinct(patient_num)) %>%
  dplyr::right_join(npat_day_all[,c("value","GROUP","days_since_admission")],by =c("GROUP","days_since_admission"))%>%
  ...
  geom_line() +
  geom_point() + 
  ylab(label = "percentage of patient without any labs")
...
plot_med_day <- LocalPatientObservations %>%
...
plot_no_med_day <- LocalPatientObservations %>%
  dplyr::filter( concept_type =="MED-CLASS") %>%
  dplyr::group_by(GROUP,days_since_admission) %>%
  dplyr::summarise(count_pat=n_distinct(patient_num)) %>%
  dplyr::right_join(npat_day_all[,c("value","GROUP","days_since_admission")],by =c("GROUP","days_since_admission"))%>%
  ...
  dplyr::mutate(per_no=round(100*(value-count_pat)/value,2))%>%
  dplyr::filter(days_since_admission >= 0 & days_since_admission <15)%>%
  ggplot(aes(x = days_since_admission, y = per_no,color = GROUP)) + 
  geom_line() +
  geom_point() + 
  ylab(label = "percentage of patient without any medication")
...
plot_pao2_day <- LocalPatientObservations %>%
  dplyr::mutate( pao2= if_else(concept_code == loinc_pao2,1,0))  %>%
  dplyr::group_by(patient_num,GROUP,days_since_admission) %>%
  dplyr::summarise(numvalue = sum(pao2))%>%
  dplyr::right_join(LocalPatientClinicalCourse[,c("patient_num","GROUP","days_since_admission")],by =c("patient_num","GROUP","days_since_admission"))%>%
...
**SENSIBILITY** : `r if (nrow(pat_ARDS)!=0 & length(unique(ADRS_Pa02$PAO2sup1))==2) {round(SEN_PaO2_ARDS,2)} else { print("No ARDS patient")}`

**PPV**         : `r if (nrow(pat_ARDS)!=0 & length(unique(ADRS_Pa02$PAO2sup1))==2) {round(PPV_PaO2_ARDS,2)} else { print("No ARDS patient")}`

**NPV**         : `r if (nrow(pat_ARDS)!=0 & length(unique(ADRS_Pa02$PAO2sup1))==2) {round(NPV_PaO2_ARDS,2)} else { print("No ARDS patient")}`

**SPECIFITY**   : `r if (nrow(pat_ARDS)!=0 & length(unique(ADRS_Pa02$PAO2sup1))==2) {round(SPE_PaO2_ARDS,2)} else { print("No ARDS patient")}`
...
**SENSIBILITY** : `r if (nrow(pat_ARDS)!=0 & length(unique(ADRS_Pa02_y$PAO2sup1))==2) {round(SEN_PaO2_ARDS_y,2)} else { print("No ARDS patient")}`

**PPV**         : `r if (nrow(pat_ARDS)!=0 & length(unique(ADRS_Pa02_y$PAO2sup1))==2) {round(PPV_PaO2_ARDS_y,2)} else { print("No ARDS patient")}`

**NPV**         : `r if (nrow(pat_ARDS)!=0 & length(unique(ADRS_Pa02_y$PAO2sup1))==2) {round(NPV_PaO2_ARDS_y,2)} else { print("No ARDS patient")}`

**SPECIFITY**   : `r if (nrow(pat_ARDS)!=0 & length(unique(ADRS_Pa02_y$PAO2sup1))==2) {round(SPE_PaO2_ARDS_y,2)} else { print("No ARDS patient")}`

```{R  message=FALSE, echo=FALSE}

# combined sensibity analysis 
...
**Patients with and without previous hospitalization in the year before **
  **Number of Patients with and without ICD-10/9 between -365 and -8 day before the admission  **
...
if (length(n_ards_y)!=0 & length(n_no_y)!=0 ) {
...
atcd_risk_per %>% datatable(rownames = FALSE)
  ...
  print(" 0 ards patient less than 50 or 0 patient without ARDS less than 50")
}
```
...
if (length(n_ards_y)!=0 & length(n_ards_o)!=0 ) {
...
plotICD_ARDS_18_49_treat
  ...
  print(" 0 ards patient less than 50 or more than 50")
}
...
**Number of patient with at least one procedure (only considering PROC-ICD10)**
...
**Status at 7 days after admission **

```{R  message=FALSE, echo=FALSE}
plot_status7 <-status7%>%
  dplyr::filter(GROUP %in% c("ARDS_18_49", "ARDS_sup_49")) %>%
...
**Status at 90 days after admission **

```{R  message=FALSE, echo=FALSE}
plot_status90 <-status90%>%
  dplyr::filter(GROUP %in% c("ARDS_18_49", "ARDS_sup_49")) %>%
  ggplot(aes(x=GROUP, y=value, fill= variable)) +
  geom_bar(stat="identity", color="black", position=position_dodge())+
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
### Elix score for commorbities comprises between -365 and -8 

```{R  message=FALSE, echo=FALSE}

plot_elix_min7 <- LocalPatientSummary  %>%
  dplyr::inner_join(comorb_elix_min7, by = "patient_num")%>%
  ggplot(aes(x = elixhauser_score)) + 
  geom_histogram(bins = 15, fill = 'slateblue')+
  facet_wrap(~GROUP, scales = "free_y",ncol = 2)
  ylab(label = "elixhauser_score")+

plot(plot_elix_min7)
```
### Elix score for commorbities comprises between -365 and +90 

```{R  message=FALSE, echo=FALSE}

plot_elix_90 <- LocalPatientSummary  %>%
  dplyr::inner_join(comorb_elix_90, by = "patient_num")%>%
  ggplot(aes(x = elixhauser_score)) + 
  geom_histogram(bins = 15, fill = 'slateblue')+
  facet_wrap(~GROUP, scales = "free_y",ncol = 2)
...
### Elix  commorbities comprises between -365 and -8 in number  
  
```{R  message=FALSE, echo=FALSE}

comorb_names_elix <- comorb_names_elix%>%
  dplyr::mutate(variable =as.character(Abbreviation))
  

out_elix_table_min7 <- out_elix%>%
  dplyr::filter( time == "before") %>%
  dplyr::inner_join(comorb_names_elix, by = "variable")%>%
  dplyr::select(Comorbidity,value ,GROUP)%>%
  group_by(GROUP) %>%
  mutate(row = row_number()) %>%
  pivot_wider(names_from = GROUP, values_from = value)%>%
  select(-row)

out_elix_table_min7 %>% datatable(rownames = FALSE)
```

### Elix  commorbities comprises between -365 and 90 in percentage 
  
```{R  message=FALSE, echo=FALSE}


out_elix_table_90 <- out_elix%>%
  dplyr::filter( time == "all") %>%
  dplyr::inner_join(num_pat,by = "GROUP")%>%
  dplyr::mutate(per = round(100*value/npat,1))%>%
  dplyr::inner_join(comorb_names_elix, by = "variable")%>%
  dplyr::select(Comorbidity,per ,GROUP)%>%
    group_by(GROUP) %>%
  mutate(row = row_number()) %>%
  pivot_wider(names_from = GROUP, values_from = per)%>%
  select(-row)

out_elix_table_90 %>% datatable(rownames = FALSE)
```
...
**Percentage of patients with pheno code after hospitalization due to covid  **
  
```{R  message=FALSE, echo=FALSE}

  compli_phe <- diag_pheno %>%
    dplyr::filter( days_since_admission >= 0) %>%
    dplyr::group_by(PheCode, GROUP,Phenotype) %>%
    dplyr::summarise(freq=n_distinct(patient_num))%>%
    dplyr::inner_join(num_pat,by = "GROUP")%>%
    dplyr::mutate(per = round(100*freq/npat,1))%>%
    dplyr::select(PheCode,Phenotype,GROUP,per)%>%
    pivot_wider(names_from = GROUP,names_sep = ".", values_from = per,names_repair = "check_unique")%>%
...
```{R  message=FALSE, echo=FALSE}
  ...
  compli_comp <- diag_comp %>%
    dplyr::filter( days_since_admission >= 0) %>%
    dplyr::group_by(complication_class, GROUP) %>%
    dplyr::summarise(freq=n_distinct(patient_num))%>%
    dplyr::inner_join(num_pat,by = "GROUP")%>%
    dplyr::mutate(per = round(100*freq/npat,1))%>%
    dplyr::select(complication_class,GROUP,per)%>%
    pivot_wider(names_from = GROUP,names_sep = ".", values_from = per,names_repair = "check_unique")%>%
    dplyr::filter(!is.na(complication_class))
  
  compli_comp %>% datatable(rownames = FALSE)
...
**Number of patient with at least one procedure ( PROC-ICD10 only ) **
...
write.csv(output_gen, file =paste("output/",siteid,"_ards_young_gen.csv",sep = ""), row.names = FALSE, na = "")
write.csv(output_day, file =paste("output/",siteid,"_ards_young_day.csv",sep = ""), row.names = FALSE, na = "")
write.csv(output_lab, file =paste("output/",siteid,"_ards_young_lab.csv",sep = ""), row.names = FALSE, na = "")
write.csv(out_proc_diag_med, file =paste("output/",siteid,"_ards_young_proc_diag_med.csv",sep = ""), row.names = FALSE, na = "")
write.csv(obfusc, file =paste("output/",siteid,"_obfusc.csv",sep = ""), row.names = FALSE, na = "")
write.csv(output_sens, file =paste("output/",siteid,"_sens.csv",sep = ""), row.names = FALSE, na = "")
